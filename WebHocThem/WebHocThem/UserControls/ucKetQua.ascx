<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ucKetQua.ascx.cs" Inherits="WebHocThem.UserControls.KetQua" %>
<asp:ListView ID="ListView1" runat="server" DataKeyNames="ID,MonHoc,Levels" DataSourceID="SqlKetQua">
    ...
    <EmptyDataTemplate>
        <h2 class="text-danger" style="text-align:center">Không có Khóa học đã học</h2>
    </EmptyDataTemplate>
    <InsertItemTemplate>
        <li style="">ID:
...
<br />
    ...
    <ItemTemplate>
        <li style="background-color: #DCDCDC;color: #000000;">ID:
            ...
            <asp:Label ID="MoTaLabel" runat="server" Text='<%# Eval("MoTa") %>' />
            <br />
            <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%#"/KetQuaKH.aspx?id="+Context.User.Identity.GetUserName()+"&kh="+Eval("ID")+"&mh="+Eval("MonHoc")+"&lv="+Eval("Levels") %>'>Kết quả khóa học</asp:HyperLink>
        </li>
    </ItemTemplate>
    <LayoutTemplate>
...
<asp:SqlDataSource ID="SqlKetQua" runat="server" ConnectionString="<%$ ConnectionStrings:DBWebHocThemConnectionString %>" SelectCommand="tKETQUA" SelectCommandType="StoredProcedure">
    <SelectParameters>
